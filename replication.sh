...
for sync_to_local in "${remote_datasets[@]}"; do
   ...
   /usr/local/sbin/syncoid ${RECVOPTS} --recvoptions="u" --sendoptions="p" --no-sync-snap --quiet \
   ...
   remote_snaps=$(ssh root@$remote_server zfs list -H -r -s name -o name -t snapshot "$sync_to_local" )
